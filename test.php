...
try {
    ...
    die('Erreur pendant la connexion avec la base de données: '.$e->getMessage());
}

// $movieIdQuery="SELECT movie_id FROM movie";
// $movieId =$pdo->query($movieIdQuery);

// //Boucle while pour récupérer les 'movie_id' dans la bdd et les mettre dans un tableau $tableMovieId
// while ($row = $movieId->fetch()) {
//     $tableMovieId[] = $row['movie_id'];
// }
// var_dump($tableMovieId);


// $stmt = $pdo->prepare("SELECT movie_name, movie_name_ov, director_firstname, director_name, movie_year FROM movie NATURAL JOIN l_director_movie NATURAL JOIN director WHERE movie_id=:movieId;");

// $arrLength=count($tableMovieId);
...
while($row=$stmt->fetch()) {?>
    ...
    <div class="movie_card_upperpart row pb-3 border border-3 align-items-evenly mx-auto">
        <div class="col-6 pt-2 border border-3 border-primary p-3">
            <img class="poster img-fluid rounded-4 " src="./assets/img/naya.png" alt="Affiche_royaume_Naya">
